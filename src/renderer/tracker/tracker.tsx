    ...
    const checkHotkeys = () => {
      overwolf.settings.hotkeys.get((result) => {
        ...
        if (result.success && result.games) {
          // Check all games for unassigned hotkeys
          const allUnassigned: string[] = [];
          Object.keys(result.games).forEach((gameId) => {
          ...
          logger.log('Unassigned hotkeys:', allUnassigned);

          if (allUnassigned.length > 0) {
            setUnassignedHotkeys(allUnassigned);
  ...
  return (
    <>
      ...
      <AppHeader
        title={
          isIngameWindow
            ? 'Overwolf App • In-Game'
            : 'Overwolf App • Desktop'
        }
            ...
            {showSettings ? (
              ...
              <div className="view-content">
                <div style={{ padding: '2rem', textAlign: 'center' }}>
                  <h2>Welcome to Overwolf App</h2>
                  <p>Use the settings button to configure the app.</p>
                </div>
              </div>
            )}
            <ReleaseNotesModal
