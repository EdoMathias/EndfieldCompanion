...
import { preloadMarkerIcons } from '../Map/utils/resource';
...
const ResourcesView: React.FC = () => {
  ...
  const [selectedNodeForMap, setSelectedNodeForMap] =
    useState<ResourceNode | null>(null);
  const containerRef = useRef<HTMLElement>(null);

  // Preload marker icons for all resource nodes
  useEffect(() => {
    const types = nodes
      .map((n) => resourceNameToMarkerType(n.name))
      .filter((t): t is string => t !== undefined);
    const uniqueTypes = [...new Set(types)];
    if (uniqueTypes.length > 0) {
      preloadMarkerIcons(uniqueTypes);
    }
  }, [nodes]);
  ...
  const stats = useMemo(() => {
    const maxedNodes = filteredNodes.filter(
      (node) => node.current >= node.max && node.max > 0,
    ).length;
    const totalGatherables = filteredNodes.reduce(
      (sum, node) => sum + node.current,
      0,
    );
  ...
  useEffect(() => {
    (
      window as unknown as { testDailyIncrement?: () => void }
    ).testDailyIncrement = runTestDailyIncrement;
    return () => {
      delete (window as unknown as { testDailyIncrement?: () => void })
        .testDailyIncrement;
    };
  }, [runTestDailyIncrement]);
      ...
      <div className="resources-regions">
        {sortedRegions.map((region) => (
          <RegionGroup
            ...
            isExpanded={
              forceFirstRegionExpanded && region === sortedRegions[0]
                ? true
                : !isRegionCollapsed(region)
            }
