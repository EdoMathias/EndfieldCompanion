...
import { resourceNameToMarkerType } from '../../../../../shared/utils/resourceMapping';
import { getMarkerIconUrl } from '../../Map/utils/resource';
  ...
  return (
    <div className="resource-node-card-body">
      ...
      <div
        ...
        style={{
          backgroundImage: node.mapLocationImage
            ? `url(${node.mapLocationImage})`
            : undefined,
        }}
      >
        <div className="resource-node-card-body-counter">
          ...
          {isEditingCurrentAmount ? (
            ...
            <span
              className="resource-node-card-body-counter-current"
              role="button"
              tabIndex={0}
              onClick={handleCurrentAmountClick}
              onKeyDown={(e) => {
                if (e.key === 'Enter' || e.key === ' ') {
                  e.preventDefault();
                  handleCurrentAmountClick();
                }
              }}
            >
              {node.current}
          ...
          {isEditingMaxAmount ? (
            ...
            <span
              className="resource-node-card-body-counter-max"
              role="button"
              tabIndex={0}
        ...
        <div className="resource-node-card-body-status">
          {/* Growing / Maxed out */}
          {isGrowing && (
            <div className="resource-node-card-body-status-growing">
              .../Growing
            </div>
          )}
          {isMaxedOut && (
            <div className="resource-node-card-body-status-maxed-out">
              .../MAX
            </div>
          )}
        </div>
          ...
          <div className="resource-node-card-body-image-container">
            {nodeIconUrl ? (
              <img
                src={nodeIconUrl}
                alt={node.name}
                className="resource-node-card-body-image-img"
                draggable={false}
              />
            ) : (
              <span className="resource-node-card-body-image-name">
                {node.name}
              </span>
            )}
          </div>
        ...
        <div className="resource-node-card-body-refresh-status">
          ...
          {isMaxedOut && (
            <div className="resource-node-card-body-refresh-status-maxed-out">
              Growth limit reached
            </div>
          )}
          {isGrowing && (
            <div className="resource-node-card-body-refresh-status-growing">
              <span className="resource-node-card-body-refresh-status-growing-label">
                Next Refresh:
              </span>
              <div className="resource-node-card-body-refresh-status-growing-counts">
                <span className="resource-node-card-body-refresh-status-growing-current-count">
                  {node.current} →{' '}
                </span>
                <span className="resource-node-card-body-refresh-status-growing-tomorrow-count">
                  {tomorrowCount}
                </span>
              </div>
            </div>
