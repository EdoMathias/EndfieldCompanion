...
export type RotationWindowSettings = {
  stepSize: number;
  opacity: number;
  bgOpacity: number;
  showSquadIndex: boolean;
};
  ...
  try {
    ...
    if (!raw)
      return {
        ...
        showSquadIndex: DEFAULT_SHOW_SQUAD_INDEX,
      };
    ...
    return {
      stepSize:
        typeof parsed.stepSize === 'number'
          ? Math.max(0.6, Math.min(1.4, parsed.stepSize))
          : DEFAULT_STEP_SIZE,
      opacity:
      ...
      bgOpacity:
        typeof parsed.bgOpacity === 'number'
          ? Math.max(0, Math.min(1, parsed.bgOpacity))
          : DEFAULT_BG_OPACITY,
      showSquadIndex:
        typeof parsed.showSquadIndex === 'boolean'
          ? parsed.showSquadIndex
          : DEFAULT_SHOW_SQUAD_INDEX,
    };
  } catch {
    return {
      stepSize: DEFAULT_STEP_SIZE,
...
function RotationStepNodeReadOnly({
  step,
  index,
  characterLabel,
}: {
          ...
          {!hasAction ? (
            ...
            <>
              ...
              <div className="rotation-step-node-action-type">
                {typeConfig?.shortLabel ?? step.action?.name ?? '—'}
              </div>
            </>
          )}
        </div>
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const rotationHotkey = hotkeysMap.get(
          kHotkeys.toggleRotationIngameWindow,
        );
        const text = displayHotkey(
          rotationHotkey?.binding,
          rotationHotkey?.IsUnassigned ?? true,
        );
    ...
    <div className="rotation-window">
      <div className="rotation-window-content">
        <div className="rotation-window-controls">
          ...
          <span className="rotation-window-hotkey-reminder">{hotkeyText}</span>
          {showSettings && (
            <div
            ...
            >
              <div className="rotation-window-settings-panel-inner">
                ...
                <label className="rotation-window-settings-label rotation-window-settings-label--checkbox">
                  <input
                    ...
                    onChange={(e) =>
                      applySettings({
                        ...settings,
                        showSquadIndex: e.target.checked,
                      })
                    }
                ...
                <button
                  type="button"
                  className="rotation-window-settings-close"
